    ...
    ?>
        ...
        <div <?php comment_class(); ?>>
            ...
            <div class="comment-start-one left">
                <div class="comment-images"><?php echo get_avatar($comment,$size='65' ); ?></div><!--/comment-images-->
...
<!-- COMMENT FORM -->
    ...
    <div class="form left" style="margin-top: 40px">
            ...
            <h2 class="comment-leave-a-comment"><?php if(isset($_GET['replytocom'])){ _e('Leave a Reply', tk_theme_name);}else{ _e('Leave a Comment', tk_theme_name);}?></h2>
            ...
            <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform contact-form" onSubmit="return checkForm(this);">
                <?php if ( !$user_ID ){?>
                    <div class="form-input left">
                        <div class="bg-input left"><span><label for="author"><?php _e('Name and Surname:', tk_theme_name); ?></label></span><input class="contact_input_text" type="text" onfocus="if(value==defaultValue)value=''" onblur="if(value=='')value=defaultValue" name="author" id="contactname" value=""/><div class="down-border-form left"></div><!--/down-border-form--></div>
                        <div class="bg-input left"><span><label for="author"><?php _e('E-mail:', tk_theme_name); ?></label></span><input class="contact_input_text" type="text" onfocus="if(value==defaultValue)value=''" onblur="if(value=='')value=defaultValue" name="email" id="contactemail" value=""/><div class="down-border-form left"></div><!--/down-border-form--></div>
                    </div>
                <?php }?>
                    <span><label for="author"><?php _e('Message:', tk_theme_name); ?></label></span><div class="form-textarea"><textarea onfocus="if(value==defaultValue)value=''" onblur="if(value=='')value=defaultValue" name="comment" id="contactmessage" ></textarea><div class="down-border-form left"></div></div>
                    <div id="contact-error"></div>

                    <div class="search-submit-button left">
                        <input class="blog-send-button"  type="submit" name="submit-comment" value="<?php _e('Send', tk_theme_name)?>" />
                    </div>
